...
public class Framework extends drawingpanel{
	...
	private void load(){
		try {
			URL burl=this.getClass().getResource("desert.png"); 
			b1 = ImageIO.read(burl); 
			
			URL turl=this.getClass().getResource("title.png"); 
			title = ImageIO.read(turl); 
			
			URL murl=this.getClass().getResource("menu_border.png"); 
			menuborder = ImageIO.read(murl); 
			
			URL curl=this.getClass().getResource("cloud_layer_1.png"); 
			cloud = ImageIO.read(curl); 
			
			URL blackurl=this.getClass().getResource("black.jpg"); 
	...
	public void Draw(Graphics2D g2d){
		switch (gamestate){
			...
			case main_menu: 
				...
				g2d.drawString("use a, d to moove the tank, and left mouse button to fire bullet, right mouse button to file rockets." , width/2-310,  height/2);
				g2d.drawString("press any key to start the game, or ESC to exit.", width/2-140, height/2+30);
				break; 
			case options:
				//......
				break; 
