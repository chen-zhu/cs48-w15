...
1) Building/executing (should work for Windows, Mac, and Linux/Unix)
	...
	b) At the command line, type in "ant" or "ant compile" to compile all source files. Source code can
	   be found in TankCommand/src/TC. Class files can be found in TankCommand/bin/TC. Upon compilation, 
	   resource files (i.e. images and sound files) will be copied into a resources directory in the bin
	...
	d) To generate a javadoc, type in "ant javadoc" at the command line. After generating it, you should 
	   be able to view it here: http://www.cs.ucsb.edu/~YOURUSERNAME/cs56/TankCommand/javadoc/ 
	   (where YOURUSERNAME is your CSIL username)

2) To add/modify data files

	Adding/modifying source code: Any source code that is modified/added should be kept within the 
				      same directory of the rest of the source files.
...
3) Special information
	...
	*When playing the game, you are not allowed to resize the window. The window where the game is played is at a
	 fixed size.
...
4) How to play
	...
	killing as many enemies as you can. Use your standard bullets to shoot enemies. Use your powerup/superpower/rocket
	...
	Change Music: Press the Change Music button at the top of the Main Menu to select a background music.
